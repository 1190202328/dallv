...
class VideoRecord(object):
    def __init__(
        ...
        self.sports_da = sports_da
        self.hmdb_ucf = hmdb_ucf
    ...
    def num_frames(self):
        if self.epic_kitchens:
            ...
            frames = [join(path, f) for f in listdir(path) if is_img(f)]
            return len(frames) - 1

    @property
    def label(self):
        return int(self._data[3]) if self.epic_kitchens else int(self._data[1])

    @label.setter
    def label(self, value):
        if self.epic_kitchens:
            self._data[3] = int(value)
        else:
            self._data[1] = int(value)
    @property
    def start_frame(self):
        return int(self._data[1]) if self.epic_kitchens else None

    @property
    def stop_frame(self):
        return int(self._data[2]) if self.epic_kitchens else None
...
def randAugment(transform_train, config):
    transform_train.transforms.insert(
        0, GroupTransform(RandAugment(config.randaug.N, config.randaug.M))
    )
